...
//    		LOG(FATAL) << " World model handle could not be initialized.";
    	...
    	if(clen == 0) {
    		LOG(INFO) << "rsg_sender: No store_dot_files configuation given. Turned off by default.";
    	} else {
    		if (*store_dot_files == 1) {
    			LOG(INFO) << "rsg_sender: store_dot_files turned on.";
    			inf->wm->scene.attachUpdateObserver(inf->wm_printer);
    	    	inf->wm_printer->setIsActive(true); // toggle visualizer ON
    		} else {
    			LOG(INFO) << "rsg_sender: store_dot_files turned off.";
    	...
    	if(clen == 0) {
    		LOG(INFO) << "rsg_sender: No store_history_as_dot_files configuation given. Turned off by default.";
    	} else {
    		if (*store_history_as_dot_files == 1) {
    			LOG(INFO) << "rsg_sender: store_history_as_dot_files turned on.";
    			inf->wm_printer->setKeepHistory(true);
    		} else {
    			LOG(INFO) << "rsg_sender: store_history_as_dot_files turned off.";
    		...
    		if(clen == 0) {
    			LOG(INFO) << "rsg_sender: No dot_name_prefix configuation given. Selecting a default name.";
    		} else {
				if(strcmp(chrptr, "")==0) {
					LOG(INFO) << "rsg_sender: dot_name_prefix is empty. Selecting a default name.";
				} else {
					std::string prefix(chrptr);
					LOG(INFO) << "rsg_sender: Using dot_name_prefix = " << prefix;
...
int rsg_sender_start(ubx_block_t *b)
{
    	...
    	if(clen == 0) {
    		LOG(INFO) << "rsg_sender: No log_level configuation given.";
    	} else {
    		if (*log_level == 0) {
    			LOG(INFO) << "rsg_sender: log_level set to DEBUG level.";
    			brics_3d::Logger::setMinLoglevel(brics_3d::Logger::LOGDEBUG);
    		} else if (*log_level == 1) {
    			LOG(INFO) << "rsg_sender: log_level set to INFO level.";
    			brics_3d::Logger::setMinLoglevel(brics_3d::Logger::INFO);
    		} else if (*log_level == 2) {
    			LOG(INFO) << "rsg_sender: log_level set to WARNING level.";
    			brics_3d::Logger::setMinLoglevel(brics_3d::Logger::WARNING);
    		} else if (*log_level == 3) {
    			LOG(INFO) << "rsg_sender: log_level set to LOGERROR level.";
    			brics_3d::Logger::setMinLoglevel(brics_3d::Logger::LOGERROR);
    		} else if (*log_level == 4) {
    			LOG(INFO) << "rsg_sender: log_level set to FATAL level.";
    			brics_3d::Logger::setMinLoglevel(brics_3d::Logger::FATAL);
    		} else {
    			LOG(INFO) << "rsg_sender: unknown log_level = " << *log_level;		}
    	}
...
void rsg_sender_step(ubx_block_t *b)
{
        ...
        LOG(INFO) << "rsg_sender: Resending the complete RSG now.";
