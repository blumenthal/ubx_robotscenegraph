...
int rsg_reciever_start(ubx_block_t *b)
{
    	...
    	if(clen == 0) {
    		LOG(INFO) << "rsg_reciever: No log_level configuation given.";
    	} else {
    		if (*log_level == 0) {
    			LOG(INFO) << "rsg_reciever: log_level set to DEBUG level.";
    			brics_3d::Logger::setMinLoglevel(brics_3d::Logger::LOGDEBUG);
    		} else if (*log_level == 1) {
    			LOG(INFO) << "rsg_reciever: log_level set to INFO level.";
    			brics_3d::Logger::setMinLoglevel(brics_3d::Logger::INFO);
    		} else if (*log_level == 2) {
    			LOG(INFO) << "rsg_reciever: log_level set to WARNING level.";
    			brics_3d::Logger::setMinLoglevel(brics_3d::Logger::WARNING);
    		} else if (*log_level == 3) {
    			LOG(INFO) << "rsg_reciever: log_level set to LOGERROR level.";
    			brics_3d::Logger::setMinLoglevel(brics_3d::Logger::LOGERROR);
    		} else if (*log_level == 4) {
    			LOG(INFO) << "rsg_reciever: log_level set to FATAL level.";
    			brics_3d::Logger::setMinLoglevel(brics_3d::Logger::FATAL);
    		} else {
    			LOG(INFO) << "rsg_reciever: unknown log_level = " << *log_level;		}
    	}
...
void rsg_reciever_step(ubx_block_t *b)
{
        ...
        struct rsg_reciever_info *inf = (struct rsg_reciever_info*) b->private_data;
        LOG(DEBUG) << "rsg_reciever: Processing an incoming update";
